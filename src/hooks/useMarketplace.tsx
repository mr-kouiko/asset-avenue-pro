  ...
  return marketplaceData
    .map((item: any) => {
      ...
      if (contentType === "vfx") {
        ...
        const hasVideoPreviewMetadata = files.find(
          (f: any) =>
            f.metadata &&
      ...
      const isAiGenerated =
        ...
        typeof originalFile.metadata === "object" &&
      ...
      return {
        ...
        duration:
          originalFile?.metadata && typeof originalFile.metadata === "object" && "duration" in originalFile.metadata
            ? (originalFile.metadata.duration as string)
            : undefined,
        bpm:
          originalFile?.metadata && typeof originalFile.metadata === "object" && "bpm" in originalFile.metadata
    ...
    async (currentFilters: MarketplaceFilters) => {
      ...
      try {
        ...
        const { data: rpcData, error } = await (supabase.rpc as any)("search_marketplace", {
          ...
          p_number_of_people_tags: currentFilters.numberOfPeopleTags?.length ? currentFilters.numberOfPeopleTags : null,
        ...
        const [creatorsResult, filesResult, likesResult, downloadsResult, translationsResult] = await Promise.all([
          ...
          supabase
            .from("content_files")
            .select(
              "id, submission_id, file_name, file_path, file_type, file_format, is_original, is_preview, preview_path, thumbnail_path, metadata",
            )
            .in("submission_id", submissionIds),
          (supabase as any)
            .from("content_like_counts")
            .select("submission_id, like_count")
            .in("submission_id", submissionIds),
        ...
        const creatorMap = new Map(
          (creatorsResult.data as any[])?.map((c) => [c.user_id, c.store_name || c.display_name]) || [],
        );
        ...
        const processed = processMarketplaceData(
          rpcData,
          filesBySubmission,
          creatorMap,
          likesMap,
