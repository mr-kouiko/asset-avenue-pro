CREATE OR REPLACE FUNCTION public.search_marketplace(
  ...
  p_resolution_tags text[] DEFAULT NULL::text[],
  p_loopable_tags text[] DEFAULT NULL::text[],
  p_copy_space_tags text[] DEFAULT NULL::text[],
  p_duration_tags text[] DEFAULT NULL::text[]
)
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE cs.status = 'approved'
      ...
      AND (p_resolution_tags IS NULL OR cs.tags && p_resolution_tags)
      AND (p_loopable_tags IS NULL OR cs.tags && p_loopable_tags)
      AND (p_copy_space_tags IS NULL OR cs.tags && p_copy_space_tags)
      AND (p_duration_tags IS NULL OR cs.tags && p_duration_tags)
