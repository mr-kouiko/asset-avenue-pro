-- Corriger la récursion infinie dans les politiques RLS user_roles
...
CREATE POLICY "Sellers can view their submissions and admins all" 
...
-- Permettre aux vendeurs/créateurs d'insérer du contenu (correction du rôle)
...
WITH CHECK (
  ...
  (public.has_role(auth.uid(), 'seller'::app_role) OR public.has_role(auth.uid(), 'admin'::app_role))
);

-- Corriger la politique de mise à jour pour les vendeurs
DROP POLICY IF EXISTS "Creators can update their pending submissions" ON public.content_submissions;

CREATE POLICY "Sellers can update their pending submissions" 
ON public.content_submissions 
FOR UPDATE 
TO authenticated
USING (
...
-- Corriger la politique de vue pour les vendeurs
...
-- Corriger les politiques content_files pour utiliser seller au lieu de creator
DROP POLICY IF EXISTS "Creators can manage their submission files" ON public.content_files;

CREATE POLICY "Sellers can manage their submission files" 
...
USING (
  EXISTS (
    ...
    AND (public.has_role(auth.uid(), 'seller'::app_role) OR public.has_role(auth.uid(), 'admin'::app_role))
  )
);