...
DECLARE
  ...
  v_audio_category_id uuid := '0b9e322e-cecb-494f-ba8d-c5397e913b99';
BEGIN
  RETURN QUERY
  WITH base AS (
    ...
    WHERE cs.status = 'approved'
      AND EXISTS (
        ...
        WHERE cf.submission_id = cs.id
          AND (
            cf.preview_quality = 'preview_available'
            OR cs.category_id = v_audio_category_id
            OR cf.file_type LIKE 'audio%'
          )
      )
