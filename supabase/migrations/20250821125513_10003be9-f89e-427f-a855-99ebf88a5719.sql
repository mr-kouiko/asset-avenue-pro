-- Fix the security definer issue by handling existing policies properly
-- Drop all relevant policies first
DROP POLICY IF EXISTS "Public can view files for approved content" ON public.content_files;
DROP POLICY IF EXISTS "Files accessible for approved products" ON public.content_files;
DROP POLICY IF EXISTS "Public read access to approved submissions" ON public.content_submissions;

-- Drop the problematic view
DROP VIEW IF EXISTS public.marketplace_content CASCADE;

-- Recreate the view with SECURITY INVOKER (the secure approach)
...
-- Create secure policies for underlying tables
CREATE POLICY "Public can access approved submissions"
ON public.content_submissions
FOR SELECT
USING (status = 'approved');

CREATE POLICY "Public can access files for approved content"
...
-- Ensure public access to creator display info for approved content
CREATE POLICY "Public can view creator info for approved content"
ON public.profiles
FOR SELECT
USING (
    EXISTS (
