-- Fix the digest function issue by using alternative hash approach
-- Replace problematic views with secure functions to eliminate SECURITY DEFINER issues
...
AS $$
    SELECT 
        ...
        p.store_name,
        p.avatar_url,
        -- Use a simple hash based on the UUID instead of digest
        substring(replace(p.user_id::text, '-', ''), 1, 16) as creator_hash
    FROM profiles p
...
AS $$
    SELECT 
        ...
        -- Use simple hash instead of digest function
        substring(replace(cs.creator_id::text, '-', ''), 1, 16) AS creator_hash,
